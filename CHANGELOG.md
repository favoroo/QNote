...
- **#128**
  - **Changed**: 深度重构与轻量化侧边栏「同步设置」页面 UI 结构与交互流程 (`lib/pages/settings/sync_settings_page.dart`)。
    ...
    - **Changed**: 开关简化：合并「自动同步」与「启动时自动同步」心智模型，开启自动同步默认包含应用启动及后台定时同步；仅在开启自动同步时展示精简的「同步频率」下拉选择；保留「同步附件图片」核心开关。
    - **Changed**: 按钮精简：将原先分散并列的 6 个操作（连接测试、手动同步、同步图片、全量同步、清理旧备份、从远程恢复）精简为唯一的「立即同步」核心行动按钮（增量同步数据与图片一体化），并将低频的「强制全量同步」、「清理旧备份」及高风险的「从云端恢复本地数据」收纳进默认折叠的「高级选项与数据恢复」面板中，显著提升初学者的易用性与界面美观度。
...
- **#119**
  - **Added**: 笔记导入图片时先插入"正在处理图片..."加载占位段，再异步执行压缩保存，完成后用真实路径替换占位段，消除"选完图回到笔记界面卡几秒空白"的感知 (`lib/widgets/notes/note_editor_view.dart`)。
    - **Added**: `_ImageSegment` 新增 `loading` 字段标记处理中状态。
    - **Changed**: `_pickImage` 重构为"先插占位、后异步压缩替换"流程，用对象引用 `identical` 定位占位段避免 index 漂移，处理失败时移除占位段。
    - **Changed**: 拆分出 `_insertImageSegment(imgSeg, {transient})` 方法，`transient: true` 的占位段不进撤销历史栈、不触发自动保存。
    - **Added**: `_buildImageSegment` 对 `loading` 段渲染固定高度加载卡片（圆角 + 进度条 + 文案），期间禁用删除/手势避免与压缩回调竞态。
  - **Changed**: `UnifiedImage` 的 `Image.file` 增加 `frameBuilder`，图片首次解码期间显示 loading、解码完成后 250ms 淡入，消除解码期空白闪烁，对日记页/全屏画廊等所有调用方均为正向优化 (`lib/widgets/unified_image.dart`)。

- **#118**
  - **Added**: 添加 iOS 平台支持，生成 ios/ 目录及配置文件 (`ios/`)。
  - **Changed**: 配置 Bundle Identifier 为 `com.appone.qnote`，应用显示名为 QNote (`ios/Runner.xcodeproj/project.pbxproj`, `ios/Runner/Info.plist`)。
  - **Changed**: 最低 iOS 版本设为 15.0 (`ios/Runner.xcodeproj/project.pbxproj`, `ios/Podfile`)。
  - **Changed**: 添加相机和相册权限声明 (`ios/Runner/Info.plist`)。
  - **Changed**: 启用 iOS 图标生成 (`pubspec.yaml`)。
  - **Changed**: 升级 file_picker 至 ^11.0.2，适配 API 变更 `FilePicker.platform.pickFiles()` → `FilePicker.pickFiles()` (`pubspec.yaml`, `lib/pages/settings/data_management_page.dart`)。
  - **Changed**: 禁用 iOS SPM（因 file_picker 和 image_cropper 的 TOCropViewController 版本冲突），改用 CocoaPods 管理原生依赖 (`flutter config --no-enable-swift-package-manager`)。

- **#117**
  - **Changed**: 优化变更日志记录格式，将条目标识从 `[HH:MM]` 时间戳改为递增序号 `#N`，解决 Agent 无法获取准确时间的问题；新条目添加在顶部，序号在上一个最大序号基础上加一 (`CHANGELOG.md`, `AGENTS.md`)。
