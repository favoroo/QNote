...
- **饮食事件**：
  - `- 种类:` 建议选填或组合 `自制` / `外卖` / `堂食` / `零食` / `水果` / `饮品` / `补剂`，亦可兼顾餐别（如 `堂食午餐`、`自制家常菜`、`甜品咖啡`）。
  - `- 评价:` `健康` / `一般` / `不健康`。
  - *示例*：
    ```markdown
    ## [12:30] 丰盛双人午餐
...
- **不要写无意义的 `- 标签:` 长串**：不要直接写 `- 标签: 午餐, 家常菜, 聚餐, 甜品`，这样会导致界面把一整段文本作为孤立字段展示；应当拆入标准维度，如 `- 分类: 饮食`、`- 种类: 堂食午餐`、详细菜品放入 `- 内容:`。
- **自由细化的正确位置**：允许在 `- 种类:`、`- 类型:` 的值中个性化填写（例如 `- 种类: 聚餐甜品`、`- 类型: 户外徒步`），也可以增加具体自定义指标（如 `- 步数: 8500`、`- 饮水: 600ml`），系统均能智能识别并良好兼容。
''';
