...
- **标准分类与标签体系（强规范）**：
  ...
  - 二级标签规范：优先填充标准字段（如饮食填 `- 种类:` 自制/堂食/外卖/饮品，活动填 `- 类型:` 工作/运动/社交，记账填 `- 支出类型:` / `- 金额:`）。
  - **兼顾自由发挥**：允许在 `- 种类:`、`- 类型:` 的值中个性化细化（如 `堂食午餐`、`户外散步`）或增加具体指标行（如 `- 步数: 8000`）；**严禁直接拼接 `- 标签: a, b, c` 这种自由长串**，这会导致界面标签展示杂乱。
