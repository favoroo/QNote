...
final contextFilterProvider = StateProvider<AiContextFilter>((ref) => const AiContextFilter());

final aiConfigListProvider = AsyncNotifierProvider<AiConfigListNotifier, List<AiConfig>>(() {
...
final chatSessionListProvider = AsyncNotifierProvider<ChatSessionListNotifier, List<ChatSession>>(() {
...
final currentChatProvider = StateNotifierProvider<CurrentChatNotifier, ChatSession?>((ref) {
...
class CurrentChatNotifier extends StateNotifier<ChatSession?> {
  ...
  Future<String> exportContext() async {
    ...
    if (filter.scope == 'all' || filter.scope == 'date' || filter.scope == 'mixed') {
      List<DiaryRecord> records;
      if (filter.startDate != null && filter.endDate != null) {
        records = await diaryRepo.getByDateRange(filter.startDate!, filter.endDate!);
      } else {
    ...
    if (filteredDiary.isNotEmpty) {
      ...
      for (int i = 0; i < filteredDiary.length; i++) {
        ...
        if (r.bodyState != null && r.bodyState!.isNotEmpty) {
          ...
          final name = bs['name'] ?? '';
          final severity = bs['severity'] ?? '';
          final duration = bs['duration'] ?? '';
          final triggers = bs['triggers'] is List ? (bs['triggers'] as List).join('、') : '';
          final notes = bs['notes'] ?? '';
  ...
  Future<void> sendMessage(String content) async {
    ...
    // 1. Prepare Time Context (in Chinese format, e.g., "2026/05/17 星期日 09:31")
    ...
    // 2. Prepare User Info (enriched with calculated age and latest weight)
    final repo = ConfigRepository.instance;
    ...
    // 4. Construct userContent for LLM (following the legacy structure exactly)
    String userContent = '';
    ...
    // 5. Append user message with raw content to active display/save history
    ...
    if (state!.title == '新对话' || state!.messages.isEmpty || 
        (state!.messages.length == 1 && state!.messages.first.role == 'assistant')) {
      newTitle = content.substring(0, content.length > 20 ? 20 : content.length);
    }
