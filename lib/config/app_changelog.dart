...
const List<ChangelogEntry> kAppChangelog = [
  ChangelogEntry(
    ...
    date: '2026-09-26',
    notes: [
      '小Q语音升级为流式朗读：长回复不再等待全部吐完，分句切片边生成边出声，首句毫秒级开口',
      '日记达成即时庆祝礼花：达成今日记录目标或刷新连击纪录时，屏幕弹出庆祝礼花与动态流光勋章',
      '评分数据支持批量调整：统计页支持按时间段加减分或设为固定值，提供逐日改动预览并支持一键撤销',
      '日记圆环连击指示优化：圆环挂载位置与粒子发射锚点对齐，记录坚持反馈更直观',
    ],
  ),
  ChangelogEntry(
    version: '0.1.72',
    date: '2026-09-26',
    notes: [
      '日记今日完整度圆环升级：顶部整条进度栏简化为输入框旁的小圆环，常驻显示今日条数与连续天数，点击一键跳转统计页',
      '统计页新增记录坚持度：直观查看当前连续、历史最长连续、连续达标天数及近30天记录概况',
      '个人资料页视觉升级：名片区采用主题色柔光渐变背景与毛玻璃昵称条，质感更精致',
      '跨零点刷新优化：跨日切回应用时自动刷新今日完整度与坚持天数',
    ],
  ),
  ChangelogEntry(
    version: '0.1.71',
  ...
  ChangelogEntry(
    ...
    date: '2026-09-25',
    notes: [
      '小米健康登录态修复：修复静默续期从未生效的问题，登录过期后能自动刷新并保存，避免整周数据同步成空',
      '睡眠分期口径修正：深睡/浅睡/快速眼动/清醒按云端枚举正确映射，并优先采用云端聚合时长，深睡不再恒为 0',
      '日记新增今日完整度进度条与记完即时庆祝：顶部显示今日记录进度与连续天数，记满或破纪录时弹出鼓励胶囊',
      '步数趋势图升级：月/年区间自动按周/月聚合不再挤成一团；日期导航条收成单行，非今天时一键回到今天',
    ],
  ),
  ChangelogEntry(
    version: '0.1.69',
  ...
  ChangelogEntry(
    ...
    date: '2026-09-21',
    notes: [
      '小米健康新增四个二级详情页：点击心率、血氧与压力、睡眠、运动记录卡片即可查看全天曲线、心率区间时长与睡眠分期时间轴',
      '修复静息心率一直显示为「--」：改从云端独立的 resting_heart_rate 指标取数，重新同步后即可看到数值',
      '指标命名更准确：原「活动时长」改为「计步分钟」，不再与小米的中高强度活动时长混淆；力量训练不再被归为「日常运动」',
      '桌面小组件添加弹窗视觉升级：大圆角卡片与胶囊按钮，带按下水波纹反馈',
    ],
  ),
  ChangelogEntry(
    version: '0.1.66',
